...
void SPI2_GPIOInits(void)
{
	...
	SPIPins.GPIO_PinConfig.GPIO_PinMode = GPIO_MODE_ALTER;
...
void SPI2_Inits(void)
{
	...
	SPI2handle.SPI_Config.SPI_BusConfig = SPI_BUSCNFG_FULLDUPLEX;
...
/*This function configures the gpio pin over which slave peripheral issues data available interrupt */
void Slave_GPIO_InterruptPinInit(void)
{
	...
	spiIntPin.GPIO_PinConfig.GPIO_Pinnumber = GPIO_PIN_N0_6;
	...
	GPIO_IRQ_Interuppt_Config(IRQ_NO_EXTI9_5,ENABLE);
...
int main(void)
{
	...
	*/
	...
	SPI_IRQ_Interuppt_Config(IRQ_NO_SPI2,ENABLE);
	...
	while(1){
		...
		GPIO_IRQ_Interuppt_Config(IRQ_NO_EXTI9_5,DISABLE);

		//enable the SPI2 peripheral using cr1 register
		...
		SPI_IRQ_Interuppt_Config(IRQ_NO_EXTI9_5,ENABLE);
...
void SPI_ApplicationEventCallback(SPI_Handle_t *pSPIHandle,uint8_t AppEv)
{
	...
	if(AppEv == SPI_RX_EVENT_COMPLETE)
	{
				RcvBuff[i++] = ReadByte;
...
{
	GPIO_IRQHandling(GPIO_PIN_N0_6);
	dataAvailable = 1;
}
